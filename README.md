...
The second finds the head of the list of classes in a room (i.e. the first element in the list) and sees if it conflicts with the proposed time by calling `pair_overlap`, which checks to see if the start and end times of two events overlap (see below).  (The two events are the proposed time to place a class and just one of the possibly many classes already placed in a room.) If they do not overlap, the tail (list minus the just checked head) is now passed back into `no_overlap` for checking as a new (shorter) list that is gradually heads toward the empty `[]` list that will satisfy the terminal clause.
